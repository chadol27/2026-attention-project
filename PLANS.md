...
2. 과제는 원래 답변에 연결된 `tasks` 배열로 같은 채팅 JSON에 저장한다.
3. 과제 화면은 같은 페이지의 모드 전환으로 표시하며, 상단에 상황에 맞는 `과제 넘어가기` 또는 `원래 대화로 돌아가기`를 표시한다.
4. 제출은 텍스트 입력 하나로 처리한다.
5. 평가 요청에는 원래 AI 답변, 과제 기준, 사용자의 제출문만 포함한다.
6. 평가 결과는 통과 여부와 짧은 피드백만 반환한다.
7. 실패하면 과제 화면을 유지하고 재제출할 수 있다.
8. 통과하면 `passed`로 저장하고 원래 대화로 돌아간다.
9. 제출 내역과 평가 결과는 모두 저장한다.
