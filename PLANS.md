...
- 일반 HTTP 접속에서 `crypto.randomUUID()`가 없는 경우 `getRandomValues()` 기반 UUID v4 생성: 반영
- 과제 화면 `Escape` 복귀와 `Ctrl+Shift+O` 새 대화 단축키: 반영
- 채팅 목록 항목 제거 기능과 원본 대화 파일 보존: 반영
- 최종 답변 생성 단계의 OpenAI 웹 검색 도구 사용: 반영
...
- 요청 형태가 아닌 기대 결과를 기준으로 한 유형 정의, 혼합 요청 우선순위와 경계 사례: 반영
...
- 요청 분류와 판정은 Chat Completions API를 사용하고, 최종 답변은 Responses API의 웹 검색 도구를 사용할 수 있게 한다.
- OpenAI 요청에는 저장용 메시지 필드 중 `role`과 `content`만 전달한다.
...
7. 목록의 `삭제`를 누르면 해당 항목을 `list.json`에서만 제거하고 개별 채팅 JSON 파일은 유지한다. 현재 대화를 제거하면 새 대화 화면으로 전환한다.
...
- 목록 삭제는 `list.json`의 항목만 제거하며 개별 채팅 파일을 삭제하지 않는다.
...
- 문제 해결 요청에 같은 대화의 이전 AI 답변이 있을 때만 별도의 호출로 `requestedDirectAnswer`를 판정하고, 그 결과를 답변 생성에 전달한다.
- 정보형 요청이면 답변 생성 전에 서버가 `검증하기`, `요약하기`, `자신만의 언어로 다시 설명하기` 중 하나를 무작위 선택해 생성 프롬프트에 전달한다.
- 요청 유형은 문장의 질문형 여부가 아니라 사용자가 기대하는 주된 결과로 분류한다. 구체적인 문제·오류 해결은 `problemSolving`, 산출물 생성·변환은 `generation`, 사용자 선택 지원은 `decision`, 지식 전달은 `information`을 우선한다.
- 여러 유형이 섞인 요청은 주된 산출물을 기준으로 분류하고, 구체적 문제 해결, 사용자 선택, 산출물 생성, 지식 설명 순으로 경계를 판별한다.
...
- 문제 해결: 첫 요청에서는 `풀어줘`, `정답을 알려줘`처럼 직접 답변을 요구해도 정답 대신 해결 힌트와 다음 단계만 제공한다. 과제는 만들지 않는다.
- 같은 문제에 대해 AI가 정답을 제외한 힌트를 제공한 뒤 사용자가 정답 또는 전체 풀이를 명시적으로 재요청한 경우에만 `requestedDirectAnswer`를 기준으로 제한을 완화한다.
- 직접 답변 판정 프롬프트에도 문제별 순서를 명시하여 첫 요청은 `false`, 힌트 이후 같은 문제의 명시적 재요청은 `true`, 새로운 문제는 다시 `false`로 판정한다.
- 답변, 과제, 평가 피드백은 사용자 입력 언어를 따른다.
- 최신 정보, 외부 정보 또는 검증 가능한 정보가 유용한 요청에는 웹 검색을 사용하고 답변에 출처 이름과 URL을 포함한다.
...
5. 과제 화면에서 `Escape`를 누르면 원래 대화로 돌아가고, 어느 화면에서든 `Ctrl+Shift+O`를 누르면 새 대화를 시작한다.
...
- 빈 채팅은 저장하지 않는다.
- 채팅 파일 삭제, 제목 수정, 검색, 페이지네이션, 로그인, 스트리밍은 MVP 이후로 미룬다.
