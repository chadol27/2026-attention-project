...
```
    ...
    7. 제출은 텍스트 입력 하나로 처리한다.
    8. 평가 요청에는 원래 AI 답변, 과제 기준, 사용자의 제출문만 포함한다.
    9. 평가 결과는 통과 여부와 짧은 피드백만 반환한다.
    10. 실패하면 과제 화면을 유지하고 재제출할 수 있다.
    11. 통과하면 `passed`로 저장하고 원래 대화로 돌아간다.
    12. 제출 내역과 평가 결과는 모두 저장한다.
